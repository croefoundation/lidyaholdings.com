...
if($rate_cus){
          ...
          //지급일, 지급액, 지급회차
           ...
           //반복구문으로 1계약당 조회후 2회 디비입력
          for ($i=1; $i <3 ; $i++) {
               $dt=computeMonth($date_y,$date_m,$date_d,5*$i-10);   //날짜 말일적용하여 역으로 계산(5n-10)

               $pay_date=$dt;
               $amount=($money*$rate_cus/100)/2; //2회로 분할지급
               $period=$i."회/반기";
