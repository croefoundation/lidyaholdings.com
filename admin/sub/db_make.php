...
<script>
window.alert('데이터베이스를 수정하실것입니까? 증말로?.');
</script>


<?
$no=2;
if($no=1){ $table="member";}
if($no=2){ $table="contract";}
if($no=3){ $table="payment";}

//주의***모든 내용 삭제..테이블은 남기고 테이블 안의 줄만 전체 삭제 한후.. 다시 오토인크리먼트 초기화
...
//맨마지막을 시작값을 다시 세팅함
$query_count ="select count(*) from $table where id='pay'"; //수당테이블 전체 갯수세기
$result1= mysql_query($query_count, $connect);
$temp= mysql_fetch_array($result1);
$totals= $temp[0];

$new=$totals+1;
$sql="ALTER TABLE $table AUTO_INCREMENT=$new" ;  //$new 따옴표서도 안되고 식을 써도 안되고..홀로써야
mysql_query($sql,$connect);
